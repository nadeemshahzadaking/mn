...
const App: React.FC = () => {
  ...
  const handleCreateItem = () => {
    ...
    if (newItemModal.type === 'folder' && newItemName.includes('.')) {
      return; // Handled by UI warning now
    }
    
    const parts = newItemName.split('.');
    const extension = parts.length > 1 ? parts.pop() || '' : '';
    
    if (newItemModal.type === 'file' && (!extension || !SUPPORTED_EXTENSIONS.includes(extension.toLowerCase()))) {
      alert(`Please provide a valid extension. Supported: ${SUPPORTED_EXTENSIONS.join(', ')}`);
      return;
    }
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-2 space-y-0.5 custom-scrollbar">
            <AnimatePresence>
              {newItemModal && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="px-2 py-2 mb-2 bg-indigo-600/5 border border-indigo-600/20 rounded-lg relative z-50">
                  <div className="flex items-center gap-2 mb-1">
                    {newItemModal.type === 'folder' ? <Folder className="w-3 h-3 text-emerald-400" /> : <File className="w-3 h-3 text-indigo-400" />}
                    <span className="text-[9px] font-bold uppercase tracking-wider text-gray-500">New {newItemModal.type}</span>
                  </div>
                  <input 
                    autoFocus
                    type="text"
                    ...
                    onChange={(e) => {
                      ...
                      setNewItemName(val);
                      if (newItemModal.type === 'file') {
                        setShowExtensions(val.includes('.'));
                      }
                    }}
                    onKeyDown={(e) => e.key === 'Enter' && handleCreateItem()}
                    className={`w-full bg-transparent text-xs outline-none text-white placeholder:text-gray-600 ${newItemModal.type === 'folder' && newItemName.includes('.') ? 'text-red-400' : ''}`}
                  />
                  
                  {newItemModal.type === 'folder' && newItemName.includes('.') && (
                    <p className="text-[9px] text-red-400 mt-1 font-medium">Folders cannot contain dots (.)</p>
                  )}

                  {newItemModal.type === 'file' && showExtensions && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {SUPPORTED_EXTENSIONS.map(ext => (
                        <button 
                          key={ext}
                          onClick={() => {
                            const base = newItemName.split('.')[0];
                            setNewItemName(`${base}.${ext}`);
                            setShowExtensions(false);
                          }}
                          className="px-1.5 py-0.5 bg-white/5 hover:bg-indigo-600/20 rounded text-[9px] text-indigo-400 border border-white/5 transition-colors"
                        >
                          .{ext}
                        </button>
                      ))}
                  ...
                  <div className="flex justify-end gap-2 mt-2">
                    <button onClick={() => { setNewItemModal(null); setShowExtensions(false); }} className="text-[10px] hover:text-white">Cancel</button>
                    <button 
                      onClick={handleCreateItem} 
                      disabled={newItemModal.type === 'folder' && newItemName.includes('.')}
                      className={`text-[10px] font-bold ${newItemModal.type === 'folder' && newItemName.includes('.') ? 'text-gray-600 cursor-not-allowed' : 'text-indigo-400'}`}
                    >
                      Create
                    </button>
                  </div>
        ...
        <AnimatePresence>
          {isPreviewVisible && (
            <motion.section 
              ...
              dragMomentum={false}
              onDragEnd={(_, info) => {
                const newX = previewPos.x + info.offset.x;
                const newY = previewPos.y + info.offset.y;
              ...
              animate={isPreviewFullscreen ? {
                ...
                position: 'fixed', top: previewPos.y, left: previewPos.x, width: 450, height: 600, zIndex: 500, x: 0, y: 0
              } : {
